...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.setpoint < 45:  # Setpoint Lower than range 45 to 65
            self.setpoint = 45  # Thermostat setpoint in limit of range
            Devices[2].Update(nValue=self.nvalue, sValue="45")
        if self.setpoint > 65:  # Setpoint Higher than range 45 to 65
            self.setpoint = 65  # Thermostat setpoint in limit of range
            Devices[2].Update(nValue=self.nvalue, sValue="65")
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
           ...
           self.cooling = False
            ...
            Domoticz.Log("cooling forced - Hardware Temp : {}ºC , fan is on".format(self.intemp))
            self.autocooling = False
            self.cooling = True
            ...
            if self.PLUGINstarteddtime + timedelta(minutes=2) < now:
                if self.intemp > self.setpoint :
                    ...
                    if self.startfan + timedelta(minutes=10) < now: # we wait a minimum of 10 minutes before stopping fan
                        if self.intemp < self.setpoint - 15 or self.intemp < 40 :
                            if not Devices[3].nValue == 0:
                                Devices[3].Update(nValue=0, sValue=Devices[3].sValue)
                                cmd = 'raspi-gpio set 26 op dl'
                                os.system(cmd)
                ...
                Domoticz.Log("cooling Auto - Plugin initilization - Fan is off ".format(self.intemp,
                                                                                                          self.setpoint))
