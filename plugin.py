...
"""
"""
<plugin key="SVT2" name="AC Smart Virtual colling control for casaia hardware" author="Erwanweb" version="0.q.1" externallink="https://github.com/Erwanweb/collingcontrol.git">
    <description>
...
class BasePlugin:
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if Unit in (1, 2, 3): # force recalculation if control or mode or a setpoint changed
            self.onHeartbeat()
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
           if not Devices[3].nValue == 0:
               Devices[3].Update(nValue = 0,sValue = Devices[7].sValue)
           ...
           if Devices[3].nValue == 0:
              Devices[3].Update(nValue = 1,sValue = Devices[7].sValue)
            ...
            if self.intemp > self.setpoint :
                if not Devices[3].nValue == 1:
                    Devices[3].Update(nValue=1, sValue=Devices[73].sValue)
                    cmd = 'raspi-gpio set 26 op dh'
                    os.system(cmd)

            else:
                if self.intemp < self.setpoint - 10 :
                    if not Devices[3].nValue == 0:
